...
router.post("/:id/create",async(req,res,next)=>{
    try{
        const checkComentario= await Comentario.find({book:req.params.id,user:req.session.user._id})
        //se comenta para dejar a los usuarios interactuar entre ellos
     //   if (checkComentario===undefined) // si no hay comentarios de ese usuario en ese libro, añadirá un comentario
     //   {
