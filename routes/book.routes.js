    ...
    const searchBook = await Book.find(
      // para poder buscar por titulo, genero o autor añadida una expresion regular para buscar por mayuscula o minuscula indistintamente
      {
        $or: [
          {
            title: {
              $regex: new RegExp(req.query.datosBusqueda.toLowerCase(), "i"),
            },
          },
          {
            author: {
...
router.post("/:id/prestado", async (req, res, next) => {
  try {
    ...
    if (foundBook.stock > 0) {
      // verificamos que haya stock antes de hacer el prestamo
      ...
      }); // resta uno al stock actual
      res.redirect("/private/profile");
    } else {
      // volvera a redirigir a la pagina del libro que el usuario queria pedir prestado y reflejara que no hay stock
...
router.post("/:id/like", async (req, res, next) => {
  try {
    ...
    const booksLike = await Book.findByIdAndUpdate(
      req.params.id,
...
router.post("/:id/nolike", async (req, res, next) => {
  try {
    // añade un usuario al array like de books y checkea que no este duplicado
    const booksNoLike = await Book.findByIdAndUpdate(
      req.params.id,
